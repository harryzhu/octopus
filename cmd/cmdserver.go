...
func init() {
	...
	serverCmd.PersistentFlags().StringVar(&UploadDir, "upload-dir", "data/uploads", "temp dir for uploads")
	serverCmd.PersistentFlags().StringVar(&StaticDir, "static-dir", "data/static", "static dir for no zstd files")
	...
	serverCmd.PersistentFlags().BoolVar(&WithR2, "with-r2", true, "if enable mongodb")
	serverCmd.PersistentFlags().BoolVar(&WithMemcache, "with-memcache", true, "if enable mongodb")
